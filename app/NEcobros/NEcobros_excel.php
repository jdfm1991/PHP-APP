...
$sheet->setCellValue('A1', 'Notas de Entregas por Cobrar por Ruta');
...
$sheet->setCellValue('A7', ('Ruta'))
    ...
    ->setCellValue('F7', ('Total 0 a 7 Dias'))
    ->setCellValue('G7', ('Total 8 a 15 Dias'))
    ->setCellValue('H7', ('Total 16 a 40 Dias'))
    ->setCellValue('I7', ('Total Mayor a 7 Dias'))
    ->setCellValue('J7', ('Saldo Pendiente $'))
    ->setCellValue('K7', ('Supervisor'));
...
$query = $NEcobros->getNotasPorCobrar($edv);
 ...
 if (is_array($query)==true and count($query)>0)
{
    ...
    foreach ($query as $i) {
            ...
            if($i["DiasTransHoy"]>=0 AND $i["DiasTransHoy"]<=7){


                if($i['SaldoPend']>=1000){

                    $sheet->setCellValue('F' . $row, ($i['SaldoPend']));
                    ...
                    $sheet->setCellValue('F' . $row, number_format($i['SaldoPend'],2));
                    $sheet->setCellValue('G' . $row, 0);
                    $sheet->setCellValue('H' . $row, 0);
                    $sheet->setCellValue('I' . $row, 0);
                    $SaldoPend_07 += $i['SaldoPend'];
                ...
                if($i["DiasTransHoy"]>=8 AND $i["DiasTransHoy"]<=15){
                   ...
                   $sheet->setCellValue('F' . $row, 0);
                    ...
                    if($i["DiasTransHoy"]>=16 AND $i["DiasTransHoy"]<=40){
            ...
            $sheet->setCellValue('J' . $row, ($SaldoPend_07+$SaldoPend_815+$SaldoPend_164+$SaldoPend_m40));
    ...
    $sheet->setCellValue('F' . $row, number_format($total_SaldoPend_07,2));
    $sheet->setCellValue('G' . $row, number_format($total_SaldoPend_815,2));
    $sheet->setCellValue('H' . $row, number_format($total_SaldoPend_164,2));
    $sheet->setCellValue('I' . $row, number_format($total_SaldoPend_m40,2));
    $sheet->setCellValue('J' . $row, number_format($SaldoPend,2));
...
header('Content-Disposition: attachment;filename="Notas_de_Entregas_por_Cobrar_por_Ruta.xlsx"');
