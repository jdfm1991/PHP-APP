...
/** DATOS DEL REPORTE **/
$spreadsheet->getActiveSheet()->getStyle('A1:L1')->getFont()->setSize(25);

if($tipo=='H'){
    
    $sheet->setCellValue('A1', 'Estado de Cuenta de Factura Proveedor');
}else{
    if($tipo=='J'){
        
    $sheet->setCellValue('A1', 'Estado de Cuenta de Notas de Entregas Proveedor');
    }
}
...
$sheet->setCellValue('A10', utf8_decode('Tipo de Transaccion'))
    ...
    ->setCellValue('H10', "Diras Transcurridos")
    ->setCellValue('I10', "Descripción")
    ->setCellValue('J10', "Débitos")
    ->setCellValue('K10', "Créditos")
    ->setCellValue('L10', 'Saldo');
...
$spreadsheet->getActiveSheet()->duplicateStyle($style_title, 'A10:L10');
...
foreach ($query as $i) {
    ...
    $sheet->setCellValue('A5', 'Proveedor :'.$i["CodProv"]);
            ...
            $sheet->setCellValue('B' . $row, $i["CodProv"]);
            ...
            $datos_pago = $cuenta->get_documento_pago($fechai, $fechaf,$cliente,$tipo,$i["NumeroD"]);

            $documento_pago='';
            $DiasTrans=$i["DiasTrans"];
            $bandera_pago=false;
    
                foreach ($datos_pago as $row_pago) {
                    if($i["NumeroD"]==$row_pago["NumeroN"]){
             ...
             if($tipoDoc=="PAG" or $tipoDoc=="Nota de Crédito" or $tipoDoc=="RET"){
                   ...
                   $sheet->setCellValue('H' . $row, "");
                    ...
                    $sheet->setCellValue('G' . $row, $documento_pago);
                    if($bandera_pago){
        
                        $sheet->setCellValue('H' . $row, "");
        
                    }else{
                        $sheet->setCellValue('H' . $row, $DiasTrans);
        
                    }

            }

            $sheet->setCellValue('I' . $row, $i["Document"]);
              ...
              $sheet->setCellValue('K' . $row, $mtotald);
            ...
            $sheet->setCellValue('L' . $row, number_format($saldo, 2, ',', '.'));

            $sheet->setCellValue('G5', 'Saldo Actual :'.number_format($saldo, 2, ',', '.'));
