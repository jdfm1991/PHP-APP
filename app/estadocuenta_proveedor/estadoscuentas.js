...
function listar_proveedor() {
    let isError = false;
    $.ajax({
        url: "estadoscuentas_controlador.php?op=listar_proveedor",
        method: "POST",
        dataType: "json",
        beforeSend: function () {
        ...
        success: function (data) {
            if(!jQuery.isEmptyObject(data.listar_proveedor)){
                //lista de seleccion de proveedor
               // $('#cliente').append('<option name="" value="Todos">Todos</option>');
                $.each(data.listar_proveedor, function(idx, opt) {
                    //se itera con each para llenar el select en la vista
                    $('#cliente').append('<option name="" value="' + opt.CodProv +'">'+opt.CodProv+' - '+ opt.Descrip+ '</option>');
                });
            }
        },
        complete: function () {
