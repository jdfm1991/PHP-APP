...
$(document).on("click", "#btn_listadepreciodivisas", function () {
    ...
    if (estado_minimizado) {
        ...
        if (depos !== "" && marcas !== "" && orden !== "") {
            ...
            $.ajax({
                async: true,
                url: "listadepreciodivisas_controlador.php?op=listar",
                method: "POST",
                dataType: "json",
...
function listar_depositos_marcas(){
    let isError = false;
    $.ajax({
        url: "listadepreciodivisas_controlador.php?op=listar_depositos_marcas",
        method: "POST",
        dataType: "json",
        beforeSend: function () {
            SweetAlertLoadingShow();
