	...
	public function getfacturasinliquidar($fechai, $fechaf, $chofer, $tipo){
       ...
       parent::set_names();
                ...
                if($tipo === '0' and $chofer === 'Todos'){
                    ...
                    from [AJ].[dbo].appfacturas_det inner join [AJ].[dbo].appfacturas on appfacturas.correl = appfacturas_det.correl inner join [AJ].[dbo].SAFACT
                    on appfacturas_det.numeros = SAFACT.NumeroD inner join [AJ].[dbo].appChofer on appfacturas.cedula_chofer = appChofer.cedula  
                    where DATEADD(dd, 0, DATEDIFF(dd, 0, appfacturas.fechad)) between '$fechai' and '$fechaf'
                    and appfacturas_det.numeros in (select numerod from [AJ].[dbo].saacxc where tipocxc='10' and saldo>0) 
                    ...
                    if($tipo === '1' and $chofer === 'Todos'){
                        ...
                        from [AJ].[dbo].appfacturas_det inner join [AJ].[dbo].appfacturas on appfacturas.correl = appfacturas_det.correl inner join [AJ].[dbo].SAFACT
                         on appfacturas_det.numeros = SAFACT.NumeroD inner join [AJ].[dbo].appChofer on appfacturas.cedula_chofer = appChofer.cedula  
                        where  DATEADD(dd, 0, DATEDIFF(dd, 0, appfacturas.fechad)) between '$fechai' and '$fechaf'
                        and appfacturas_det.numeros not in (select numerod from [AJ].[dbo].saacxc where tipocxc='10' and saldo>0) 
                        order by fechad, appfacturas_det.numeros";
                    }else{
