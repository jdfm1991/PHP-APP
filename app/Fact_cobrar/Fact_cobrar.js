 ...
 //   var marcas = $('#marca').val();
    ...
    if (estado_minimizado) {
        ...
        if (datos) {
            ...
            $.ajax({
                url: "Fact_cobrar_controlador.php?op=listar",
                method: "POST",
                dataType: "json",
                data: datos,
                beforeSend: function () {
    ...
    if(!jQuery.isEmptyObject(data.contenido_tabla)){
        ...
        $('#Fact_cobrar_data')
            .append(
                ...
                '<td><strong>' + data.totales_tabla.SaldoPendolar + '</strong></td>' +
                '<td>  </td>' +
                '</tr>'
            );
        ...
        $('#Fact_cobrar_data').append('<tr><td colspan="11" align="center">Sin registros para esta Consulta</td></tr>');
...
function listar_marcas() {
    let isError = false;
    $.ajax({
        url: "Fact_cobrar_controlador.php?op=listar_marcas",
        type: "get",
        dataType: "json",
        beforeSend: function () {
            SweetAlertLoadingShow();
...
$(document).on("click","#btn_excel", function(){
    ...
    if (datos !== "") {
        window.location = 'Fact_cobrar_excel.php?&'+datos;
    }
});
