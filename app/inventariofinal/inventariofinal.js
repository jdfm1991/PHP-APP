...
function listar_almacenes() {
    let isError = false;
    $.ajax({
        url: "inventariofinal_controlador.php?op=listar_depositos",
        type: "GET",
        dataType: "json",
        beforeSend: function () {
...
$(document).on("click", "#btn_inventariofinal", function () {
    ...
    if (estado_minimizado) {
        ...
        estado_minimizado = false;
        if (depo.length > 0 && fechai !== "" && fechaf !== "") {
            ...
            $.ajax({
                url: "inventariofinal_controlador.php?op=listar_inventariofinal",
                type: "POST",
                data: datos,
                dataType: "json",
                beforeSend: function () {
                ...
                success: function(data) {
                    if(!jQuery.isEmptyObject(data)) {
                        ...
                        $('#inventariofinal_data').append('<tr><td colspan="8" align="center">Sin registros para esta Consulta</td></tr>');
                    }
                },
                complete: function () {
...
$(document).on("click","#btn_excel", function(){
    ...
    if (datos !== "") {
        window.location = 'inventariofinal_excel.php?&'+datos;
    }
});
...
$(document).on("click","#btn_pdf", function(){
    var datos = sessionStorage.getItem("datos");
    if (datos !== "") {
        window.open('inventariofinal_pdf.php?&'+datos, '_blank');
    }
});
