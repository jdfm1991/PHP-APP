...
class Principal extends Conectar{
    ...
    public function getDocumentosSinDespachar($fini,$ffin){
...
$sql="SELECT NumeroD, Descrip, TipoFac from safact AS SA where DATEADD(dd, 0, DATEDIFF(dd, 0, SA.FechaE))
                        between '$fini' and '$ffin' and CodVend not in ('01') and SA.TipoFac in ('A','C') and
    ...
    public function get_cxp_dolares(){
       ...
       /* $sql = "SELECT SUM(total-abono) as saldo_dolares
        ...
        $sql = "SELECT sum(SAACXP.Saldo) as saldo_dolares from CONFIMANIA_D.dbo.SAACXP
           where CONFIMANIA_D.dbo.SAACXP.saldo>0 AND (CONFIMANIA_D.dbo.SAACXP.tipocxp='10' OR SAACXP.tipocxp='20')" ;
...
$sql="SELECT YEAR(CAST(SAITEMNOTA.FechaE AS DATETIME)) anio, MONTH(CAST(SAITEMNOTA.FechaE AS DATETIME)) mes,
    ...
    public function get_detalle_almacen_producto($alm,$insta) {
        ...
        //QUERY
                ...
                $sql="    SELECT  CantEmpaq AS display, CostAct AS costo, SUM(saexis.existen) AS cantidad_b,SUM(saexis.exunidad) AS cantidad_p
                FROM saprod INNER JOIN saexis ON saprod.codprod = saexis.codprod
                WHERE (saexis.existen > 0 OR saexis.exunidad > 0) AND len(marca) > 0 AND saexis.codubic IN ('$alm') AND marca = '$insta'
    ...
    public function get_ventas_por_marca_fact($fechai, $fechaf){
       ...
       /* $sql = "SELECT marca,
        ...
        $sql="SELECT
                ...
                (SAITEMFAC.TotalItem/Tasai) as montod
                 FROM SAITEMFAC INNER JOIN saprod ON SAITEMFAC.coditem = saprod.codprod
                 INNER JOIN SAFACT ON SAITEMFAC.numerod = SAFACT.numerod AND SAITEMFAC.tipofac = SAFACT.tipofac WHERE
                 DATEADD(dd, 0, DATEDIFF(dd, 0, SAITEMFAC.FechaE)) between ? AND ? AND (SAITEMFAC.tipofac = 'A' OR SAITEMFAC.Tipofac = 'B') ORDER BY SAITEMFAC.fechae asc
...
";
     ...
     public function get_ventas_dia($fechai, $fechaf){
        //LLAMAMOS A LA CONEXION QUE CORRESPONDA CUANDO ES SAINT: CONEXION2
        //CUANDO ES APPWEB ES CONEXION.
        $conectar= parent::conexion2();
        parent::set_names();
        ...
        declare @rutas_fact VARCHAR(1000)
        declare @rutas_dev VARCHAR(1000)
        SET @EDV = 'TODOS'
        set @fechai = GETDATE()
        set @fechaf = GETDATE()
        select
        CONVERT(varchar, (sum(case when TipoFac = 'A' then (monto/nullif(tasa, 0)) when TipoFac = 'B' then (monto/nullif(tasa, 0)) * -1 end) -
        sum(case when TipoFac = 'A' then ((Descto1 + Descto2)/nullif(tasa, 0)) when TipoFac = 'B' then ((Descto1 + Descto2)/nullif(tasa, 0))*-1  end) ), 1) dolar,
        CONVERT(varchar, (sum(case when TipoFac = 'A' then monto when TipoFac = 'B' then (monto) * -1 end) -
        sum(case when TipoFac = 'A' then (Descto1 + Descto2) when TipoFac = 'B' then ((Descto1 + Descto2))*-1  end) ), 1) bolivares
        from
        SAFACT where TipoFac in ('A','B') and DATEADD(dd, 0, DATEDIFF(dd, 0, safact.fechae)) BETWEEN @fechai and @fechaf";

        /*$sql="select sum(monto) as bolivares, sum(monto/nullif(tasa, 0)) as dolar from SAFACT where DATEADD(dd, 0, DATEDIFF(dd, 0, FechaE)) between ? and ? and TipoFac='A' and  NumeroR IS NULL";*/
    ...
    public function get_ventas_dia_dolares($fechai, $fechaf){
        //LLAMAMOS A LA CONEXION QUE CORRESPONDA CUANDO ES SAINT: CONEXION2
        //CUANDO ES APPWEB ES CONEXION.
        $conectar= parent::conexion2();
        parent::set_names();
        ...
        set @fechaf = GETDATE()
        select
        CONVERT(varchar, (sum(case when TipoFac = 'C' then ((total/1.16)) end) ), 1) Vendido,
        CONVERT(varchar, (sum(case when TipoFac = 'D' then total/1.16 when TipoFac = 'C' then 0 end) ), 1) Devoluciones,
        CONVERT(varchar, (sum(case when TipoFac = 'C' then (nullif(descuento, 0)/1.16) when TipoFac = 'D' then (nullif(descuento, 0))*-1  end) ), 1) Descuento,

        CONVERT(varchar, (sum(case when TipoFac = 'C' then ((total/1.16)*tasa) end) ), 1) Vendidobs,
CONVERT(varchar, (sum(case when TipoFac = 'D' then ((total/1.16)*tasa) when TipoFac = 'C' then 0 end) ), 1) Devolucionesbs,
CONVERT(varchar, (sum(case when TipoFac = 'C' then ((nullif(descuento, 0)/1.16)*tasa) when TipoFac = 'D' then (nullif(descuento, 0))*-1  end) ), 1) Descuentobs
        from
        SANOTA where TipoFac in ('C','D') and  DATEADD(dd, 0, DATEDIFF(dd, 0, SANOTA.fechae)) BETWEEN @fechai and @fechaf";
