...
class DespachosRelacion extends Conectar{
    ...
    public function getRelacionDespachos() {
        ...
        //QUERY
        $sql= "select TOP 200 * from appfacturas inner join [APPWEB_CONFIMANIA].[dbo].[Usuarios] on appfacturas.id_usu = [APPWEB_CONFIMANIA].[dbo].[Usuarios].Cedula order by correl desc";
    ...
    public function get_despacho_por_correlativo($correlativo){
        ...
        //QUERY
        $sql= "SELECT * from appfacturas inner join [APPWEB_CONFIMANIA].[dbo].[Usuarios] on appfacturas.id_usu = [APPWEB_CONFIMANIA].[dbo].[Usuarios].Cedula where correl = ? order by correl";
    ...
    public function get_detalle_clientes_por_correlativo($correlativo) {
        ...
        //QUERY
        $sql = "SELECT SACLIE.CodClie, SACLIE.Descrip, (case when safact.TipoFac = 'C' then (MtoTotal / NULLIF(safact.tasa,0)) else MtoTotal end) as MtoTotal FROM SACLIE inner join safact on safact.CodClie=SACLIE.CodClie where NumeroD = '$correlativo'";
    ...
    public function get_productos_devueltos_de_un_despacho($correlativo) {
        ...
        $sql = "SELECT DISTINCT CodItem, Descrip,
                    ...
                    EsUnid = 0 AND TipoFac = 'B' AND OTipo = 'A' AND (ONumero IN ( SELECT Numerod FROM Despachos_Det WHERE ID_Correlativo = ? ))), 0)
                    AS BULTOS,
                    COALESCE((SELECT SUM(Cantidad) FROM SAITEMFAC WHERE CodItem = SAPROD.CodProd AND
                    EsUnid = 1 AND TipoFac = 'B' AND OTipo = 'A' AND (ONumero IN ( SELECT Numerod FROM Despachos_Det WHERE ID_Correlativo = ? ))), 0)
                    AS PAQUETES,
                    CantEmpaq,
                    EsEmpaque,
                    saprod.Tara AS tara,
                    ...
                    TipoFac = 'B' AND OTipo = 'A' AND (ONumero IN ( SELECT Numerod FROM Despachos_Det WHERE ID_Correlativo = ? )) ORDER BY SAITEMFAC.CodItem";
    ...
    public function get_facturas_devueltas_de_un_despacho($correlativo) {
        ...
        $sql = "SELECT DISTINCT ONumero
				...
				WHERE TipoFac = 'B' AND OTipo = 'A'  AND (ONumero IN ( SELECT Numerod FROM Despachos_Det WHERE ID_Correlativo = ? )) 
