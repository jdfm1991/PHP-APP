...
switch ($_GET["op"]) {
    ...
    case "guardaryeditar_modulo":
        ...
        if (empty($id_modulo)) {
            ...
            if (is_array($datos) == true and count($datos) == 0) {
                ...
                $modulo = $gestion->registrar_modulo($data);
            ...
            $modulo = $gestion->editar_modulo($data);
        }
    ...
    case "activarydesactivar_modulo":
        ...
        if (is_array($datos) == true and count($datos) > 0) {
            ...
            //edita el estado
            $gestion->editar_estado_modulo($id, $activo);
    ...
    case "eliminar_modulo":
        ...
        if(is_array($modulo) == true and count($modulo) > 0) {
            $eliminar = $gestion->eliminar_modulo($id);
        }
    ...
    case 'guardarseleccionado':
        ...
        switch ($tipo) {
            case 'modulo':
                $guardar = $gestion->editar_menuid_en_modulo($id, $menu_id);
                break;
            case 'menu_padre':
                $guardar = $gestion->editar_menuportipo_en_menu($id, $menu_id, $tipo);
                break;
            case 'menu_hijo':
                $guardar = $gestion->editar_menuportipo_en_menu($id, $menu_id, $tipo);
                break;
    ...
    case "guardaryeditar_menu":
        ...
        importante: se debe poner el $_POST sino no funciona*/
        if (empty($id_menu)) {
            $menu = $gestion->registrar_menu($data);

        } else {
            /*si ya existe entonces editamos el usuario*/
            $menu = $gestion->editar_menu($data);
        }
    ...
    case "activarydesactivar_menu":
        ...
        if (is_array($datos) == true and count($datos) > 0) {
            ...
            ($activo == "0") ? $activo = 1 : $activo = 0;
            //edita el estado
            $gestion->editar_estado_menu($id, $activo);
            //evalua que se realizara el query
            ($estado) ? $output["mensaje"] = "Actualizacion realizada Exitosamente" : $output["mensaje"] = "Error al Actualizar";
        }
    ...
    case "eliminar_menu":
        ...
        if(is_array($menu) == true and count($menu) > 0) {
            $eliminar = $gestion->eliminar_menu($id);
        }
