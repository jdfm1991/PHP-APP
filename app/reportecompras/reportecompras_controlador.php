...
switch ($_GET["op"]) {
    ...
    case "listar":
        ...
        foreach ($codidos_producto as $key=>$coditem){
            ...
            $sub_array['displaybultos'] = Strings::rdecimal($row[0]["displaybultos"], 0);
            ...
            $sub_array['bultospenultimacompra'] = (count($compra) > 0) ? Strings::rdecimal($compra[0]["bultospenultimacompra"], 0) : 0;
            $sub_array['fechaultimacompra'] = (count($compra) > 0) ? date(FORMAT_DATE,strtotime($compra[0]["fechaultimacompra"])) : '-';
            $sub_array['bultosultimacompra'] = (count($compra) > 0) ? Strings::rdecimal($compra[0]["bultosultimacompra"], 0) : 0;
            $sub_array['semana1'] = Strings::rdecimal($row[0]["semana1"], 0);
            $sub_array['semana2'] = Strings::rdecimal($row[0]["semana2"], 0);
            $sub_array['semana3'] = Strings::rdecimal($row[0]["semana3"], 0);
            $sub_array['semana4'] = Strings::rdecimal($row[0]["semana4"], 0);
            $sub_array['totalventasmesanterior'] = Strings::rdecimal($row[0]["totalventasmesanterior"], 0);
            $sub_array['bultosexistentes'] = Strings::rdecimal($row[0]["bultosexistentes"], 1);
            $sub_array['diasdeinventario'] = Strings::rdecimal($row[0]["diasdeinventario"], 0);
