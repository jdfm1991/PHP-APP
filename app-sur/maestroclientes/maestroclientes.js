...
function listar_vendedores() {
    let isError = false;
    $.ajax({
        url: "maestroclientes_controlador.php?op=listar_vendedores",
        method: "POST",
        dataType: "json",
        beforeSend: function () {
...
$(document).on("click", "#btn_consultar", function () {
    ...
    if (estado_minimizado) {
        ...
        if (vendedor !== "") {
            ...
            tabla = $('#maestro_data').DataTable({
                "aProcessing": true,//ACTIVAMOS EL PROCESAMIENTO DEL DATATABLE.
                "aServerSide": true,//PAGINACION Y FILTROS REALIZADOS POR EL SERVIDOR.
                "ajax": {
                    url: "maestroclientes_controlador.php?op=buscar_maestroclientes",
                    type: "post",
                    dataType: "json",
...
$(document).on("click","#btn_excel", function(){
    ...
    if (vendedor !== "") {
        window.location = "maestroclientes_excel.php?&vendedor="+vendedor;
    }
});
...
$(document).on("click","#btn_pdf", function(){
    var vendedor = sessionStorage.getItem("vendedor", vendedor);
    if (vendedor !== "") {
        window.open('maestroclientes_pdf.php?&vendedor='+vendedor, '_blank');
    }
});

function mostrar() {

    var texto= 'Total Maestro Clientes: ';
