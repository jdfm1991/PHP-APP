...
function listar_rutas() {
    let isError = false;
    $.ajax({
        url: "resumencobrorutas_controlador.php?op=listar_vendedores",
        method: "POST",
        dataType: "json",
        beforeSend: function () {
    ...
    if (estado_minimizado) {
        ...
        if (fechai !== "" && fechaf !== "") {
            ...
            tabla = $('#comisiones_data').DataTable({
                ...
                "ajax": {
                    url: "resumencobrorutas_controlador.php?op=buscar_cobros",
                    type: "post",
                    dataType: "json",
...
$(document).on("click","#btn_pdf", function(){
    ...
    var fechaf = sessionStorage.getItem("fechaf", fechaf);
    if (fechai !== "" && fechaf !== "") {
        window.open('resumencobrorutas_pdf.php?&fechai='+fechai+'&fechaf='+fechaf, '_blank');
    }
});
