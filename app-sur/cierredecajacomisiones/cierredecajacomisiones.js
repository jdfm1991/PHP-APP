...
function listar_rutas() {
    let isError = false;
    $.ajax({
        url: "cierredecajacomisiones_controlador.php?op=listar_vendedores",
        method: "POST",
        dataType: "json",
        beforeSend: function () {
...
$(document).on("click", "#btn_consultar", function () {
    $("#comisiones_data").dataTable().fnDestroy();
    ...
    if (estado_minimizado) {
      //$("#tabla").show('');//MOSTRAMOS LA TABLA.
        ...
        if (fechai !== "" && fechaf !== "") {
            ...
            tabla = $('#comisiones_data').DataTable({
                "aProcessing": true,//ACTIVAMOS EL PROCESAMIENTO DEL DATATABLE.
                "aServerSide": true,//PAGINACION Y FILTROS REALIZADOS POR EL SERVIDOR.
                "ajax":  {
                    url: "cierredecajacomisiones_controlador.php?op=buscar_comisiones",
                    type: "post",
                    dataType: "json",
...
$(document).on("click","#btn_excel", function(){
   ...
   if (fechai !== "" && fechaf !== "" && ruta !== "" && tipo !== "") {
    window.location = "cierredecajacomisiones_excel.php?&fechai="+fechai+"&fechaf="+fechaf+"&ruta="+ruta+"&tipo="+tipo;
}
});
...
$(document).on("click","#btn_pdf", function(){
    ...
    var fechaf = sessionStorage.getItem("fechaf", fechaf);
    if (fechai !== "" && fechaf !== "") {
        window.open('cierredecajacomisiones_pdf.php?&fechai='+fechai+'&fechaf='+fechaf, '_blank');
    }
});
