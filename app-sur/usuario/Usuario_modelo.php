...
class Usuario extends Conectar
{
    public function registrar_usuario($data)
    {
        ...
        $sql = "INSERT INTO usuarios(Cedula, Login, Nomper, Email, Clave, ID_Rol, Fecha_Registro, Fecha_Ult_Ingreso, Estado, foto) 
                VALUES(?,?,?,?,?,?,?,?,?,?)";
    ...
    public function editar_usuario($data)
    {
        ...
        $sql = (hash_equals('', $data["clave"]))
            ? "UPDATE usuarios SET  Login=?,  Nomper=?,  Email=?,  ID_Rol=?,  Estado=? ,  foto=?  WHERE   Cedula=?"
            : "UPDATE usuarios SET  Login=?,  Nomper=?,  Email=?,  Clave=?,  ID_Rol=?,  Estado=?,  foto=?  WHERE   Cedula=?";
