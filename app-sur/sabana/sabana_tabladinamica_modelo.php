...
class Tabladinamica_Sabana extends Conectar{
    ...
    public function getCostosdEinventario($marca,$fechai,$fechaf)
	{
		//LLAMAMOS A LA CONEXION QUE CORRESPONDA CUANDO ES SAINT: CONEXION2
		//CUANDO ES APPWEB ES CONEXION.
		$conectar = parent::conexion2();
	...
	/*	$sql = "SELECT saexis.codprod AS codprod, CantEmpaq AS display, Descrip AS descrip, Tara AS tara, Marca AS marca, CostAct AS costo, precio1 AS precio, SUM(saexis.existen) AS bultos,SUM(saexis.exunidad) AS paquetes,
		(SELECT factorp from SACONF where CodSucu = 00000) as factor
                ...
                WHERE  len(marca) > 0 AND saexis.codubic IN ('01') AND saprod.CodProd LIKE '$marca'
                GROUP BY saexis.codprod, CantEmpaq, descrip, CostAct, precio1, Marca, tara";*/

    $sql="SELECT historico_costos.id, saexis.codprod AS codprod, CantEmpaq AS display, Descrip AS descrip,EsExento, Tara AS tara, Marca AS marca, historico_costos.costo AS costo, precio1 AS precio, SUM(saexis.existen) AS bultos,SUM(saexis.exunidad) AS paquetes,
		(SELECT factorp from SACONF where CodSucu = 00000) as factor
                FROM saprod INNER JOIN saexis ON saprod.codprod = saexis.codprod inner join historico_costos on historico_costos.codprod= saprod.codprod
                WHERE /*(saexis.existen > 0 OR saexis.exunidad > 0) AND*/ len(marca) > 0 AND saexis.codubic IN ('01') AND saprod.CodProd LIKE '$marca' and historico_costos.fecha BETWEEN '$fechai' AND '$fechaf'
                GROUP BY id, saexis.codprod, CantEmpaq, descrip, costo,EsExento, precio1, Marca, tara";
