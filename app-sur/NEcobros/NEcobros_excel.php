...
$sheet->setCellValue('A7', ('Ruta'))
    ...
    ->setCellValue('E7', ('Dias de Visita'))
    ->setCellValue('F7', ('Fecha Emisión'))
...
$spreadsheet->getActiveSheet()->duplicateStyle($style_title, 'A7:L7');
 ...
 if (is_array($query)==true and count($query)>0)
{
    ...
    foreach ($query as $i) {
            ...
            $sheet->setCellValue('E' . $row, $i['DiasVisita']);
            $sheet->setCellValue('F' . $row, date('d/m/Y', strtotime($i["FechaEmi"])));
            ...
            $sheet->setCellValue('L' . $row, $i['Supervisor'] );
    ...
    $spreadsheet->getActiveSheet()->getStyle('A'.$row.':L'.$row)->getFill()->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)->getStartColor()->setARGB('17a2b8');
