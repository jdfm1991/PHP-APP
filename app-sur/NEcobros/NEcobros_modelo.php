...
class NEcobros extends Conectar
{
	public function getNotasPorCobrar($edv)
	{
		...
		//QUERY
		$sql = "SELECT (case when saacxc.tipocxc = 10 then 'NE' else 'N/D' end) as TipoOpe, saacxc.numerod as NroDoc, saclie.CodClie as CodClie, saclie.Descrip as Cliente, DiasVisita,
            ...
            DATEDIFF(DD, saacxc.fechae, CONVERT( date ,GETDATE()))as DiasTransHoy,
             ...
             from DCONFISUR_PZO_D.dbo.saacxc inner join saclie on saacxc.codclie = saclie.codclie inner join SACLIE_01 on SACLIE_01.codclie = saclie.codclie
	...
	SELECT (case when saacxc.tipocxc = 10 then 'NE' else 'N/D' end) as TipoOpe, saacxc.numerod as NroDoc, saclie.CodClie as CodClie, saclie.Descrip as Cliente, DiasVisita,
          CONVERT( date , saacxc.fechae ) as FechaEmi, 
          DATEDIFF(DD, saacxc.fechae, CONVERT( date ,GETDATE()))as DiasTransHoy,
           UPPER(saacxc.codvend) as Ruta, saacxc.saldo as SaldoPend, 
           (select Coordinador from SAVEND_02 where SAVEND_02.CodVend = saacxc.CodVend) as Supervisor
