...
//        $depos =  array_map(function($val) { return $val['codubi']; }, Almacen::todos());
        ...
        foreach ($datos as $key => $row) {
            ...
            if($row["almacen"]=='01'){
                ...
                if($row["almacen"]=='02'){
                    ...
                    if($row["almacen"]=='06'){
                        ...
                        if($row["almacen"]=='07'){
                            $almacen='CHATARRA_ DAÑADO EN ALMACEN';
                            ...
                            if($row["almacen"]=='08'){
                                ...
                                $almacen='CONFIMANIA';
                                    ...
                                    if($row["almacen"]=='10'){
                                    $almacen='COMERCIALIZADORA CONFISUR BCL';
                                   } else{

                                    if($row["almacen"]=='03'){
                                        $almacen='DEVOLUCIONES EN RECEPCION DE MERCANCIA';
                                       } else{
                        ...
                        foreach ($dato_factor as $rowa) {
                            ...
                            $dato_datelle  = $principal->get_detalle_almacen($row["almacen"]);
                            $valor_t  = 0;
                            foreach ($dato_datelle as $row87) {

                              
                                $datos72 = $principal->get_detalle_almacen_producto($row["almacen"],$row87['instancia']);
            ...
            $sub_array['total']     = Strings::rdecimal(floatval($valor_t),2);
    ...
    case "buscar_total_ventas_mes_encurso":
       ...
       $datos1= $principal->get_total_ventasbolivares($fechai, $fechaf);
        ...
        foreach ($datos1 as $row1) {
                    ...
                    if($row1['tipo']=='A'){
                        $totalbs += ($row1["Vendidobs"]/$row1["factor"]);
                    }
        }
       ...
       $datos1= $principal->get_total_ventasbolivares($fechai, $fechaf);
        ...
        $descuentos += floatval($row24["Descuento"]);
       ...
       $datos1= $principal->get_total_ventasbolivares($fechai, $fechaf);
        ...
        $output["total"] = number_format((($totald+$totalbs)-$descuentos), 2, ",", "");
        echo json_encode($output);
        break;
                 ...
                 $cdisplay = $cantidad_b= $cantidad_p= $valor_bulto= $valor_paq= $valor_t = 0;
                    ...
                    foreach ($datos72 as $row72) {
                        ...
                        if ($row72['display'] == 0) {
                            ...
                            $cantidad_b   += ((floatval($row72["cantidad_b"])));
                            $cantidad_p   += (floatval($row72["cantidad_p"]));
                        ...
                        $total_b += $valor_bulto;
                                $total_p += $valor_paq;
                                $total += $valor_t;
                                $total_cantidadb +=  $cantidad_b ; 
                                $total_cantidadp+= $cantidad_p ;
